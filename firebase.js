...
import { initAuthGuard, logoutUser, getDataById } from "./firebase.js";

// --- Route Protection Setup ---
initAuthGuard({
  protectedRoutes: ["/customer-dash.html", "/provider-details.html"],
  adminRoutes: ["/provider-dash.html"],
  ...
  onUserLoaded: async (user) => {
    ...
    if (user) {
      ...
      const userRes = await getDataById("users", user.uid);
      const userRole = userRes.success ? userRes.data.role : "customer";

      navActions.innerHTML = `
        <span class="text-light me-3 small d-none d-md-inline">Welcome, <strong>${user.name || user.email}</strong></span>
        ${userRole === "provider" ? '<a href="provider-dash.html" class="btn btn-outline-warning btn-sm me-2">Provider Dashboard</a>' : ''}
        ${userRole === "customer" ? '<a href="customer-dash.html" class="btn btn-outline-info btn-sm me-2">My Bookings</a>' : ''}
      ...
      navActions.innerHTML = `
        <a href="login.html" class="btn btn-primary-custom btn-sm">Sign In / Register</a>
      `;
    }
  }
});